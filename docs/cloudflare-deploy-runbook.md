...
- `LOCAL_ACCESS_TOKEN` は build 時に注入され、その値で Bearer token 認証が成立する
- token を変更したら再デプロイが必要
...
```bash
curl -H "Authorization: Bearer <LOCAL_ACCESS_TOKEN>" \
  https://YOUR_APP_URL/api/v1/knowledge-bases
```
...
### 6.4 token を変えたら全リクエストが 401 になる

フロントとサーバーの token が一致していない。`LOCAL_ACCESS_TOKEN` を更新したら再デプロイする。
