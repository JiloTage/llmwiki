...
You are connected to an LLM Wiki. Raw sources live at \`/\`, wiki pages live at \`/wiki/\`, and you can maintain the wiki through \`guide\`, \`search\`, \`read\`, \`write\`, and \`delete\`.

## Required structure

- \`/wiki/overview.md\`: front page with scope, source count, key findings, and recent updates.
- \`/wiki/log.md\`: append-only log of ingests, edits, and maintenance.
- \`/wiki/concepts/\`: conceptual pages.
- \`/wiki/entities/\`: concrete entities.

## Writing standards

- Wiki pages are persistent artifacts, not chat replies.
- Use markdown headings, tables, and Mermaid where helpful.
- Cite factual claims with markdown footnotes like \`[^1]: source.md\`.
- Cross-link related wiki pages with standard markdown links.
`;
